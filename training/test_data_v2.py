...
def _write_synthetic_npz(path: Path, n: int = 5, num_moves: int = 17, *,
    ...
    # Each position: visit distribution on 3 random moves, else 0.
    policy = np.zeros((n, num_moves), dtype=np.float16)
    for i in range(n):
        legal = rng.choice(num_moves, size=3, replace=False)
        pi = rng.random(3).astype(np.float32)
        pi /= pi.sum()
        policy[i, legal] = pi.astype(np.float16)
    ...
    legal_count = np.full((n,), 3, dtype=np.int16)
...
def test_legal_mask_matches_policy_nonzero(tmp_path: Path):
    path = tmp_path / "sample.npz"
    _write_synthetic_npz(path, n=4, num_moves=11)
    batch = load_v2_npz(path)
    assert np.array_equal(batch.legal_mask, batch.policy > 0.0)
    # Every row must have at least one legal move
    assert batch.legal_mask.sum(axis=1).min() > 0


def test_legal_mask_helper():
    policy = np.array([[0.0, 0.5, 0.0, 0.5], [0.3, 0.0, 0.7, 0.0]], dtype=np.float32)
    mask = _legal_mask_from_policy(policy)
    assert mask.dtype == bool
    assert mask.tolist() == [[False, True, False, True], [True, False, True, False]]
